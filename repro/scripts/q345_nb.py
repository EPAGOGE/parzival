...
CASES = [("NB28", 28), ("NB44", 44)]
EDGES = (0.0, 2.0, 15.0, 25.0)
DEGS, EPSB = (16, 40, 12), 1e-4
...
for tag, NB in CASES:
    ...
    if not info.get("converged"):
        log(f"[{tag}] Nb={NB}  NOT CONVERGED: {info}  [{time.time()-t0:.0f}s]")
        continue
    log(f"[{tag}] Nb={NB:3d}  converged=True  alpha={info['alpha']:+.8f}  "
    ...
    log(f"        n2 = {S.Nx*S.Nb}   N = {3*S.Nx*S.Nb+2}")
    del ss, L
log("DONE")
