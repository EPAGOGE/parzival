"""POLAR_SPECTRUM: linear stability of the converged corner profile.

Integrated 2026-07-28 from the verified prototype.  ZERO edits to
polar_cornerreg.py -- this file is additive and imports the solver.

WHAT IT PRODUCED (record: ALPHA_RESULT.md section 8; graded claims: NOTE_CLAIMS.md
rows S1-S10):
  * Hurwitz on the corner-clamped admissible class, certified WITHOUT trusting an
    eigenvalue -- Lyapunov L^T P + P L = -I, relative residual 4.85e-16 / 4.88e-16 /
    4.68e-16 at three roots, cholesky(P) succeeding at all three.
  * No eps-pseudospectrum reaches Re z > 0 below eps* = 3.93e-3, against a round-off
    floor of 1.175e-9 (margin 3.3e6).  The VERDICT survives refinement; the eps*
    LEVEL moves 21.5% across degrees and is quoted with a band.
  * NOT claimed: unconditional linear stability.  The Lyapunov rate 8.82e-6 is 3.6e4
    below the Schur abscissa 3.17e-1, and the admissible class is corner-clamped.

TWO WARNINGS THIS FILE EXISTS TO ENFORCE:
  * Do NOT deflate the grading direction.  The DAE restriction to ker(Cg) already
    removes it (sigma_min 2.5356e-6 -> 3.9701e-3, a 1565.7x rise at three roots);
    its restricted shadow is an ORDINARY STIFF direction of the generator
    (||L w1||/||w1|| = 12.08, ~1% of ||L||), and deflating it moves the resolvent
    by ~20%.  It is an error, not a redundancy.
  * Do NOT quote an eigenvalue from this operator.  Two independent routes agree on
    the rightmost to 1.3e-9 while two grids disagree by 2.14e-1 -- seven and a half
    orders between well-posed-as-linear-algebra and converged-as-discretization.
    Every number above is eigenvalue-free for exactly that reason.

---- prototype documentation follows ----
"""
