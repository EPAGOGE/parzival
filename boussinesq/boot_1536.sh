#!/bin/bash
# N=1536 tier-one Boussinesq run, MPI-parallel, with a hard idle-burn guard.
#
...
[ "$NP" -lt 1 ] && NP=1
echo "launching N=1536 on $NP ranks $(date -u)" > LAUNCHED

mkdir -p runs
OMP_NUM_THREADS=1 /home/ubuntu/mf/bin/mpirun -n "$NP" --oversubscribe \
  /home/ubuntu/mf/bin/python3 dedalus_bsq.py \
  --Nx 1536 --Nz 1536 --A 4 --stop 3.5 \
  --run-id M1536 --checkpoint-wall 600 \
  --out runs/big_N1536.json > runs/big_N1536.log 2>&1
